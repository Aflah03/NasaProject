...
def visualize_tif_for_area(filename, bbox):
    with rasterio.open(filename) as src:
        ...
        out_image, out_transform = mask(src, [india_bbox], crop=True)
        out_image = out_image[0]  # Get the first band

        plt.imshow(out_image, cmap='gray')
        plt.colorbar()
        plt.title(f"Visualization of {filename} for India")
        plt.show()
...
# Define the bounding box for India
india_bbox = [68.7, 8.4, 97.25, 37.6]  # [min_lon, min_lat, max_lon, max_lat]

# Visualize the downloaded .tif file for India
visualize_tif_for_area('today.tif', india_bbox)